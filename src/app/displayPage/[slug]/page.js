...
export default function PrevStuff({ params }) {
  ...
  if (!editori_id) {
    return (
      <div>
        404 your data not found or rather your data is now in the under-world of
        the internet hahaaha!!
      </div>
    );
  }
  ...
  return (
    <>
      <div className="flex flex-col justify-start ">
        <div className=" text-2xl m-3 p-2">Thoughts Saved =&gt;</div>
        <div>
          ...
          <div className=" text-2xl m-3 p-2">Feelings Saved =&gt;</div>

          <div className="flex">
            {thoughtsData.feelings
              ? thoughtsData.feelings.map((thought) => {
                  return (
                    <div className="p-2 m-3 border rounded-lg border-white">
                      {thought}
                    </div>
                  );
                })
